...
def fitKineticModelTS():
    #TODO: Create Fitting function to fit time series data
    pass
    

def fitMachineLearningModels(measurements,outputs):
    baggingMinMax = Pipeline([('normalize',MinMaxScaler(feature_range=(-1, 1))),('linear',BaggingRegressor(base_estimator=DecisionTreeRegressor(),
                           n_estimators=50,
                           bootstrap=False,
                           oob_score=False))])
    baggingStandard = Pipeline([('normalize',StandardScaler()),('linear',BaggingRegressor())])
    ridge = Pipeline([('normalize',StandardScaler()),('poly',PolynomialFeatures(degree=4)),('linear',Ridge(normalize=False))])
    support_vector = Pipeline([('normalize',StandardScaler()),('svr',SVR(kernel='poly',degree=4))])
    pls = Pipeline([('linear',PLSCanonical())])
    ml_models = [RandomForestRegressor(n_estimators=100,min_samples_leaf=2), baggingStandard, baggingMinMax]

    for regr in ml_models:
        regr.fit(measurements,outputs)
...
def evaluate_ml_model(model,measurements,outputs):
    score = model.score(measurements, outputs)
    return score
...
def coverModel(model):
    return testPoints, testValues
...
def plotModel(model,data,targets,midpoint=0.1,pcs=None):
    #Visualize Model
    ...
    #print(minpoint,midpoint,maxpoint)
    ...
    plt.title('Limonene Production vs Enzyme Concentrations')
...
# def plotTimeSeries(model,time_points,data,y0,parameters):
#
#     #Create Quick to Evaluate Cost Function
#     f = lambdify(model['parameters-all']+model['measurements-all']+model['outputs'],model['equations'])
#     g = lambda x,p,t: f(*(list(p) + [y(t) for y in interplist] + list(x)))
#     ja = lambda x,p,t: model['jacobianFcn'](*(list(p) + [y(t) for y in interplist] + list(x)))
#
#     #Integrate forward And Extract Time Points
#     h = lambda x,t: g(x,p,t)
#     jac = lambda x,t: ja(x,p,t)
#     #with stdout_redirected():
#     substrate_time_series = odeint(h,y0,times,Dfun=jac)
#
#     #Plot Metabolites Vs. Predicted Metabolites
